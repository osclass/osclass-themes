        ...
        <div class="container">
            ...
            <div class="content user_forms">
                <div id="contact" class="inner">
                    <h1><?php _e('Contact seller', 'hierarchy') ; ?></h1>
                    <ul id="error_list"></ul>
                    ...
                    <form action="<?php echo osc_base_url(true); ?>" method="post" name="contact_form" id="contact_form" >
                        <fieldset>
                            ...
                            <label><?php _e('To (seller)', 'hierarchy') ; ?>: <?php echo osc_item_contact_name() ; ?></label><br/>
                            ...
                            <?php } else { ?>
                                <label for="yourName"><?php _e('Your name', 'hierarchy') ; ?></label> <?php ContactForm::your_name() ; ?><br/>
                                <label for="yourEmail"><?php _e('Your e-mail address', 'hierarchy') ; ?></label> <?php ContactForm::your_email() ; ?><br />
                            <?php } ?>
                            <label for="phoneNumber"><?php _e('Phone number', 'hierarchy') ; ?></label> <?php ContactForm::your_phone_number() ; ?><br/>
                            <label for="message"><?php _e('Message', 'hierarchy') ; ?></label> <?php ContactForm::your_message() ; ?><br />
                            <?php osc_show_recaptcha() ; ?>
                            <button type="submit"><?php _e('Send message', 'hierarchy') ; ?></button>
                        </fieldset>
                    </form>
                </div>
            </div>
