...
<?php if( osc_count_items() == 0) { ?>
    ...
    <div class="span11 columns">
        <?php if( osc_search_pattern() == '' ) { ?>
            <h5><?php _e('There are no results', 'twitter_bootstrap') ; ?></h5>
        <?php } else { ?>
...
<?php while ( osc_has_premiums() ) { ?>
    <div class="premium line span11 columns">
        ...
        <div class="description">
            ...
            <p><?php printf(__('<strong>Publish date</strong>: %s', 'twitter_bootstrap'), osc_format_date( osc_premium_pub_date() ) ) ; ?></p>
            <?php
                $location = array() ;
                if( osc_premium_country() != '' ) {
                    $location[] = sprintf( __('<strong>Country</strong>: %s', 'twitter_bootstrap'), osc_premium_country() ) ;
                }
                if( osc_premium_region() != '' ) {
                    $location[] = sprintf( __('<strong>Region</strong>: %s', 'twitter_bootstrap'), osc_premium_region() ) ;
                }
                if( osc_premium_city() != '' ) {
                    $location[] = sprintf( __('<strong>City</strong>: %s', 'twitter_bootstrap'), osc_premium_city() ) ;
                }
