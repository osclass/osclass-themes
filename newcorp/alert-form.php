    ...
    $(document).ready(function(){
        $(".sub_button").click(function(){
            $.post('<?php echo osc_base_url(true); ?>', {email:$("#alert_email").val(), userid:$("#alert_userId").val(), alert:$("#alert").val(), page:"ajax", action:"alerts" }, 
                function(data){
                    if (data==1) { alert('<?php _e('You have sucessfully subscribed to the alert', 'newcorp'); ?>'); }
                    else if (data == -1) { alert('<?php _e('Invalid email address', 'newcorp'); ?>'); }
                    else { alert('<?php _e('There was a problem with the alert', 'newcorp');?>');
                    };
            });
            return false;
        });
...
<div class="alert_form">
    ...
    <form action="<?php echo osc_base_url(true); ?>" method="post" name="sub_alert" id="sub_alert">
        <fieldset>
            ...
            <?php } ?>
            <button type="submit" class="sub_button" ><?php _e('Subscribe now!', 'newcorp') ; ?></button>
        </fieldset>
    </form>
</div>